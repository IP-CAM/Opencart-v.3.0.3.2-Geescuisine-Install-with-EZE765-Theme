...
class ControllerSimpleBlogAuthor extends Controller
{
    ...
    public function insert()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->response->redirect($this->url->link('simple_blog/author', 'user_token=' . $this->session->data['user_token'] . $url, true));
        }
        $this->getForm();
    }
    ...
    public function getList()
    {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('simple_blog/author', 'user_token=' . $this->session->data['user_token'] . $url, true),
            'separator' => ' :: '
        );
        $data['insert'] = $this->url->link('simple_blog/author/insert', 'user_token=' . $this->session->data['user_token'] . $url, true);
        $data['delete'] = $this->url->link('simple_blog/author/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
        $data['authors'] = array();
        $filter_data = array(
            'sort'  => $sort,
            'order' => $order,
        ...
        foreach ($results as $result) {
            ...
            $data['authors'][] = array(
                ...
                'edit'                  => $this->url->link('simple_blog/author/update', 'user_token=' . $this->session->data['user_token'] . '&simple_blog_author_id=' . $result['simple_blog_author_id'] . $url, true)
            );
        }
    ...
    public function getForm()
    {
        ...
        if (!isset($this->request->get['simple_blog_author_id'])) {
            $data['action'] = $this->url->link('simple_blog/author/insert', 'user_token=' . $this->session->data['user_token'] . $url, true);
        } else {
            $data['action'] = $this->url->link('simple_blog/author/update', 'user_token=' . $this->session->data['user_token'] . '&simple_blog_author_id=' . $this->request->get['simple_blog_author_id'] . $url, true);
        }
        $data['cancel'] = $this->url->link('simple_blog/author', 'user_token=' . $this->session->data['user_token'] . $url, true);
