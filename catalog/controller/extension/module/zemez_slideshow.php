...
class ControllerExtensionModuleZemezSlideshow extends Controller
{
    ...
    {
        static $module = 0;
        $this->document->addScript('catalog/view/theme/' . $this->config->get('theme_' . $this->config->get('config_theme') . '_directory') . '/js/zemez_slideshow/swmin.js');
        $this->document->addScript('catalog/view/theme/' . $this->config->get('theme_' . $this->config->get('config_theme') . '_directory') . '/js/zemez_slideshow/swinit.js', 'footer');
        $data = array(
            'width'                       => $setting['width'],
        ...
        if (isset($setting['slides'])) {
            ...
            if ($vid) {
                $this->document->addScript('catalog/view/theme/' . $this->config->get('theme_' . $this->config->get('config_theme') . '_directory') . '/js/zemez_slideshow/jquery.vide.min.js');
            }
        }
        $data['module'] = $module++;
