    ...
    n = function() {
      ...
      b.prototype.initialize     = function() {
        ...
        a.$element.wrapInner(e("<div/>", {"class" : "rd-parallax-inner"})).find(".rd-parallax-layer[data-type]").each(function() {
          ...
          switch (d.attr("data-type").toLowerCase()) {
            case "media":
              if (c = this.getAttribute("data-url")) {
                d.css({"background-image" : a.url(c)}), ("true" === this.getAttribute("data-blur") || a.options.blur) && e("<img/>", {src : c}).load(function() {
          ...
          if (!l) {
            ...
            if ("true" === this.getAttribute("data-fade") && !k) {
              a.$doc.on("scroll", e.proxy(a.fadeLayer, this, a));
            }
            if ("true" === this.getAttribute("data-fade") && !k) {
              a.$doc.on("resize",
      ...
      b.prototype.transform      = function(a, d) {
        return k && 10 > q ? {transform : "translate(0," + a + "px)"} : {
          ...
          a + "px, 0)", transform : "translate3d(0," + a + "px, 0)", transition : l ? "" + d.options.duration + "ms transform " + d.options.easing : "none"
        }
      };
      b.prototype.blur           = function(a) {
        return 3 < a ? {"-webkit-filter" : "blur(" + a + "px)", filter : "blur(" + a + "px)"} : {filter : "none", "-webkit-filter" : "none"}
      };
      b.prototype.getAttribute   = function(a, d) {
        var c, b, f, e, g;
        if (null != this.options.screenAliases) {
          for (b = Object.keys(this.options.screenAliases).reverse(), f = e = 0, g = b.length - 1; (0 <= g ? e <= g : e >= g) && !(c = "" !== this.options.screenAliases[b[f]] ? "-" + this.options.screenAliases[b[f]] :
    ...
    e.fn.extend({
      RDParallax : function(b) {
        return this.each(function() {
          var a;
          a = e(this);
          if (!a.data("RDParallax")) {
            return a.data("RDParallax", new n(this, b))
          }
        })
      }
    });
