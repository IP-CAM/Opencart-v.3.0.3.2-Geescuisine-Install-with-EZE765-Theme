...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$this->document->addStyle('catalog/view/theme/' . $this->config->get('theme_' . $this->config->get('config_theme') . '_directory') . '/stylesheet/photoswipe.css');
			...
			$data['points'] = $product_info['points'];
				...
				$pos = strpos($desc,'<p class="video-tab"><iframe');
				if (is_int($pos)) {
				$pos2 = strpos($desc, '</iframe></p>') + 9;
